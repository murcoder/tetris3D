 ...
 */
   function getShader(gl, id) {
    ...
    /**
     * Calculate radiants into degrees
     * @param degrees
     * @returns {Number}
     */
    ...
    /**
     * Save the current model-view-Matrix on the stack
     */
    var mvMatrix = mat4.create();
    var mvMatrixStack = [];
    var pMatrix = mat4.create();
    ...
    /**
     * load the last model-view-Matrix from the stack
     */
    ...
    function tetrominoConstructor(name, items,posSize,colSize, colors, vertexIndices,
    		blocks_0, objectGrid0, objectGrid90, objectGrid180, objectGrid270,   topleft, blocksFromMiddle){
	   	...
	   	tetromino = {
		    ...
		    //Grid data
		    blocks_0: blocks_0,
		    objectGrid: objectGrid0,		//Default
		    objectGrid0: objectGrid0,
		    objectGrid90: objectGrid90,
		    objectGrid180: objectGrid180,
		    objectGrid270: objectGrid270,
		    topleft: topleft, //[x=middle-1, y=top-1, z=depthMiddle-1(z is positive for grid!)]; !Topleft Also saved in resetCurrentElem()!
		    blocksFromMiddle: blocksFromMiddle, //Look how many blocks are in the direction [x,-y,+y,-z] from the middle
...
//		    					//Jump back to middleponint for after drawing single block
		    					
//			    //draw the default block on a new position
				for(var i=0; i<this.blocks_0.length; i++){
					mvPushMatrix();
					
					//[-x,+y,-z] draw around the middlepoint (default: [8,17,-8]) 
					mat4.translate(mvMatrix, [this.blocks_0[i][0],this.blocks_0[i][1],this.blocks_0[i][2]]);
	   	...
	   	printObject: function(){
			console.log("Name: " + this.name + "\n" +
					   "Rotation[x,y,z]: " + "["+ this.rotationX + ", " + this.rotationY + ", " + this.rotationZ + "] " + "\n" +
						"Position[x,y,z]: [" + this.position + "] " + "\n" + 
						"Topleft[x,y,z]:   [" + this.topleft + "] " + "\n" +
						"PotentialTopleft: [" + this.potentialTopleft + "] " + "\n" + 
						"BlocksFromMiddle[+x,-y,+y,-z]: " + this.blocksFromMiddle + "\n" +
						"[y]: OBJECTGRID [z=" + this.objectGrid.length + "]" + "\n" );
			
			for(var y=0; y<this.objectGrid[0].length; y++){
				console.log("[" + y  + "]: " + this.objectGrid[0][y] + "\n");
			}
    ...
    function drawScene() {
        ...
        if(!view2d){
	        ...
	        //2D position for the camera
	        mat4.translate(mvMatrix, [-8, -col/2, -50]);
	        
	        //2D rotation for the camera
	        mat4.rotate(mvMatrix, degToRad(0), [1, 0, 0]);
	        mat4.rotate(mvMatrix, degToRad(360), [0, 1, 0]);
    ...
    function resetCurrentElem(){
		...
		//Reset topleft
		if(elem == 0) //O
    		tetrominos[elem].topleft = [row/2-1, col-col+2, (row/2)-1];
		if(elem == 1) //T
    		tetrominos[elem].topleft = [row/2-1, col-col+2, (row/2)-1];
		if(elem == 2) //I
    		tetrominos[elem].topleft =  [row/2, col-col, (row/2)-1];
		if(elem == 3) //L
    		tetrominos[elem].topleft = [row/2-1, col-col+2, (row/2)-1];
		if(elem == 4) //S
    		tetrominos[elem].topleft = [row/2-1, col-col+2, (row/2)-1];
    ...
    function copyObject(orig){
    	var copy = tetrominoConstructor( orig.name, orig.items, orig.posSize, orig.colSize, orig.colors,  orig.vertexIndices, orig.blocks_0,
    			orig.objectGrid0, orig.objectGrid90, orig.objectGrid180, orig.objectGrid270,  orig.topleft, orig.blocksFromMiddle);
    ...
    function reset(){
	   ...
	   if(!resetView){
		   ...
		   deltaYrotate = 0;
		   scale = 1;
		   setMessages("MOUSE LOCKED: Press 'Q' or 'Reset' again to release the mouse control!");
	   }
	   else{
		   resetView = false;
		   setMessages(" ");
